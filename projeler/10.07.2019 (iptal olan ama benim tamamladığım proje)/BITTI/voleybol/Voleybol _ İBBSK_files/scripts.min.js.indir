...
$(document).ready(function () {
    ...
    const infoBar = $('header .info-bar');
    const filters = $('.page-wrapper section');
    const filterDropDown = $('.filter-dropdown');
    const filterActive = $('.filter-dropdown .title b');
    const filterItems = $('.filter-dropdown ul');
    const filterItem = $('.filter-dropdown li');
    const HeadTabsTitle = $('.head-links .head-mobile-btn');
    const HeadTabsItem = $('.head-links a');
    const FilterSearch = $('.filter-search');
    const activitiesBranchSlider = $('.activities-branch-slider');
    const navTextIcon = $('.nav-text-icon');
    const mobileNavIcon = $('.mobile-nav-icon');
    const mobileMenu2 = $('#menu-wrapper .mobile-menu');
    const menuClose = $('#menu-wrapper .menu-close');
    const fixedSearch = $('.fixed-search');
    const websiteSearch = $('input#website_search');
    const branchMobileNavIcon = $('.branch-mobile-nav-icon');
    $('header .contact-us').on('click', function () {
        if (headerBox.hasClass('hide')) {
    ...
    $('.circle-contact').on('click', function () {
    ...
    HeadTabsTitle.on('click', function () {
        if ($(this).next('.links').css('display') === 'block') {
            $(this).find('i').removeClass('fa-angle-up');
            $(this).find('i').addClass('fa-angle-down');
            $(this).next('.links').hide();
        } else {
    ...
    filterItem.on('click', function () {
        ...
        filterActive.parent('.title').attr('data-active', dataId);
        filterActive.html(dataTitle);
        $('.score-table').removeClass('active');
        $('.score-table[data-id=' + dataId + ']').addClass('active')
    });
    HeadTabsItem.on('click', function () {
        const dataId = $(this).data('id');
        const dataTitle = $(this).text();
        HeadTabsTitle.attr('data-active', dataId);
        HeadTabsTitle.html(dataTitle + ' <i class="fa fa-angle-down"></i>');
        if (filters.width() <= 791) {
            $(this).parent('.links').hide();
        } else {
            $(this).parent('.links').show();
            HeadTabsItem.removeClass('active');
            $(this).addClass('active');
        }
    });
    $('.newsletters-branch').slick({
        dots: false,
        arrows: true,
        infinite: false,
        centerPadding: '0',
    ...
    $('#custom-search').on('click', function () {
        FilterSearch.toggleClass('active');
    });
    $('.featured-video .play').on('click', function () {
        $(this).parent('.content').fadeOut();
    });
    activitiesBranchSlider.slick({
        slidesToShow: 5.6,
        slidesToScroll: 1,
        dots: false,
        infinite: false,
    ...
    if (screen.width <= 991) {
        branchMobileNavIcon.parent('a').parent('.logo').siblings('nav').slideUp();
    }
    branchMobileNavIcon.on('click', function () {
        $('#vertical-menu').find('nav').slideToggle();
    });
    const mobileMenu = $('#mobile-menu');
    const mobileStatic = mobileMenu.children('.static');
    const mobileDynamic = mobileMenu.find('.dynamic');
    const mobileToggle = mobileMenu.find('.toggle,.search');
    const menuItem = mobileMenu.find('.menu-item');
    const menu = $('.menu-item.menu');
    const menuClubs = $('.menu-item.clubs');
    const menuMedia = $('.menu-item.mobile-media:not(.clubs)');
    const menuBranch = $('.menu-item.mobile-branch');
    const menuLink = menu.find('a');
    const search = $('.menu-item.mobile-search');
    const menuFoot = mobileMenu.find('.foot');
    const mobileSearch = $('#mobile_search');
    const results = mobileMenu.find('.results');
    const customSearchOpen = $('.custom-search-open');
    const searchFilter = mobileMenu.find('.filter');
    const mobileSearchItem = searchFilter.find('li');
    const calendarAdd = $('.calendar-add');
    const popup = $('#popup');
    const teamPlayers = $('#team-page .players li');
    ...
    mobileToggle.on('click', function (e) {
        ...
        if (e.currentTarget.className === 'search') {
            ...
            menuFoot.addClass('hidden');
        }
        if (e.currentTarget.className === 'toggle') {
            menu.addClass('active');
            menuFoot.removeClass('hidden');
        }
        if (mobileMenu[0].className !== 'active') {
            $('html,body').addClass('disable');
            mobileToggle.find('.close-menu').show();
            mobileMenu.addClass('active');
            mobileStatic.addClass('hidden');
            mobileDynamic.addClass('active');
            results.removeClass('active');
            searchFilter.removeClass('active');
            customSearchOpen.addClass('active');
        } else {
            $('html,body').removeClass('disable');
            mobileMenu.removeClass('active');
            mobileToggle.find('.close-menu').hide();
            mobileStatic.removeClass('hidden');
            mobileDynamic.removeClass('active');
            customSearchOpen.removeClass('active');
        }
    });
    ...
    mobileSearchItem.on('click', function (e) {
    ...
    menuLink.on('click', function () {
        const dataBox = $(this).data('box');
        menuItem.removeClass('active');
        if (dataBox === 'clubs') {
            menuClubs.addClass('active');
        } else if (dataBox === 'media') {
            menuMedia.addClass('active');
        } else if (dataBox === 'branch') {
            menuBranch.addClass('active');
        }

    });
    customSearchOpen.on('click', function () {
        searchFilter.addClass('active');
        $(this).removeClass('active');
    });
    $('.close-register').on('click', function () {
        $('.contact-mobile').addClass('disabled');
    });
    $('.contact-open').on('click', function () {
        $('.contact-mobile').removeClass('disabled');
    });
    calendarAdd.on('click', function () {
        const googleDate = $(this).data('date');
        const title = $(this).data('title');
        const branch = $(this).data('branch');
        popup.find('.button').attr('href', 'http://www.google.com/calendar/event?action=TEMPLATE&text=' + title + ' - ' + branch + '&dates=' + googleDate + '/' + googleDate + '')
        popup.removeClass('hidden');
        popup.find('.default').show();
    ...
    $(document).mouseup(function (e) {
        activitePopup = $('#activities-page #popup');
        activitePopupDefault = $('#activities-page #popup .default');
        if (activitePopupDefault) {
            if (!activitePopupDefault.is(e.target) && activitePopupDefault.has(e.target).length === 0) {
                activitePopup.addClass('hidden');
            }
        }
    });

    HeadTabsItem.on('click', function () {
        $('.team-content .players p').remove();
        const thisPosition = $(this).data('position');
        teamPlayers.hide();
        if (thisPosition === 'all') {
            teamPlayers.show();
        } else {
            $('#team-page .players li[data-position=' + thisPosition + ']').show();
        }
        mess = true
        $('.team-content .players li').each(function (index, val) {
        ...
        if (mess === true) {
            setTimeout(function () {
                $('.team-content .players').append("<p style='width: 100%; text-align: center;margin-top:60px;'>Bu Pozisyon da Hiç Kimse Bulunamadı !</p>")
            }, 300)
    ...
    function create_bulletin(input_elem, message_elem) {
        $.ajax({
            url: "bulten/",
            type: "POST",
            data: {email: input_elem.val()},
            success: function (data, i) {
                input_elem.val('');
                if (data[0].text.email) {
                    message_elem.html("<p style='visibility: visible'>" + data[0].text.email + "</p>");
                } else {
                    message_elem.html("<p style='visibility: visible'>" + data[0].text + "</p>");

                }
            },
            error: function (xhr, errmsg, err) {
                message_elem.html("<p style='visibility: visible'>Bir sorun oluştu! Lütfen daha sonra tekrar deneyiniz.</p>");
                console.log(xhr);
            }
        })
    }
    ...
    function create_contact() {
        ...
        $.ajax({
            ...
            success: function (json) {
                console.log(json);
                $('.contact-message').html("<p>Mesajınız Gönderildi! İlginiz için Teşekkürler.</p>");
            },
            error: function (xhr, errmsg, err) {
                $('.contact-message').html("<p>Oops! Mesajınız Gönderilemedi.</p>");
                console.log(xhr.status + ": " + xhr.responseText)
            }
        })
    }

    const headerBullettin = $('.header-bulletin');
    const headerBullettinMobile = $('.header-bulletin-mobile');
    const bullettinMessageHeader = $('.bulletin-message_header');
    const footerBullettin = $('.footer-bulletin');
    $('#header-bulletin').on('submit', function (e) {
        e.preventDefault();
        headerBullettin.val();
        create_bulletin($('.header-bulletin'), bullettinMessageHeader);
    });

    $('#header-bulletin-mobile').on('submit', function (e) {
        e.preventDefault();
        headerBullettinMobile.val();
        create_bulletin(headerBullettinMobile, bullettinMessageHeader);
    });

    $('#footer-bulletin').on('submit', function (e) {
        e.preventDefault();
        footerBullettin.val();
        create_bulletin(footerBullettin, $('.bulletin-message_footer'));
    });
    ...
    const videoPromotional = $('[data-promotional]');
    const promotionClick = $('#promotional-film .video-cover');
    videoPromotional.on('click', function () {
        if ($('body').attr('id') === 'home-page') {
            $('html, body').animate({scrollTop: promotionClick.offset().top - 100}, 2000);
            setTimeout(function () {
    ...
    $('#share_on_twitter').on('click', function () {
        postToTWTTR();
    });
    $('#share_on_facebook').on('click', function () {
        var page_title = $('.page-title h1').text();
        postToFB(page_title);
    });

    let title = $('#share_title').text();
    if (title == '' || title == 'undefinied')
        title = $.trim($("title").text());
    title = 'İBBSK | ' + title;

    const body = encodeURIComponent(location.href);
    $('#share_via_mail').attr('href', `mailto:?subject=${title}&body=${body}`);
    ...
    const search_label = $('#menu-wrapper .search-wrapper form label');
    const search_input = $('#menu-wrapper .search-wrapper form label input');
    search_button.on('click', function () {
        fixedSearch.addClass('open');
        body2.css({'overflow': 'hidden'})
    });
    mobileNavIcon.click(function () {
        if (mobileMenu2.hasClass('open')) {
            mobileMenu2.removeClass('open');
            body2.css('overflow', 'auto');
        } else {
            mobileMenu2.addClass('open');
            body2.css('overflow', 'hidden');
        }
    });
    menuClose.click(function () {
        if (mobileMenu2.hasClass('open')) {
            mobileMenu2.removeClass('open');
            body2.css('overflow', 'auto');
            navTextIcon.removeClass('open');
            mobileMenu2.find('.sub-open-menu').removeClass('open');
        }
    });
    navTextIcon.click(function () {
        if ($(this).hasClass('open')) {
            $(this).removeClass('open');
            $(this).parent('.nav-item').find('.sub-open-menu').removeClass('open');
        } else {
            $('.nav-text-icon.open, .sub-open-menu.open').removeClass('open');
            $(this).addClass('open');
            $(this).parent('.nav-item').find('.sub-open-menu').addClass('open');
        }
    });
    body2.css({"visibility": "visible", "opacity": "1"});
    setTimeout(function () {
        preLoader.hide();
    }, 3600);
    searchClose.on('click', function () {
        ...
        body2.css({'overflow': 'auto'})
        websiteSearch.val('');
        $('#results').addClass('hidden').children('ul').find('li').remove()
    });

    function autoHeight_all_view() {
        const newsListWrapper = $('.news-list-wrapper');
        const buttonHeight = $('.all-views').height();
        const titleHeight = $('.news-list-vertical-title').height();
        const divNum = $('.news-list-wrapper .swiper-slide').length;
        if (divNum < 4) {
            newsListWrapper.css('min-height', 'calc(100% - ' + (buttonHeight + titleHeight + 15) + 'px)');
            if (screen.width > 991) {
                $('.news-list-wrapper .news-list-item').css({'height': 'auto'})
            }
        }
    }
    ...
    try {
        ...
        if (imgBg.find('.active').offset().top > 0) {
            var offsetTop = imgBg.find('.active').offset().top;
            imgBg.animate({
                scrollTop: offsetTop
            }, 850);
        }
    ...
    if (bodyID === 'score-page') {
        if (id) {
            if (id === '#basketbol') {
                //$('.filter-dropdown li ').data('id', 'basketbolligi').trigger('click')
                const dataId = 'basketbolligi';
                const dataTitle = 'Tahincioğlu Basketbol Ligi';
                filterActive.parent('.title').attr('data-active', dataId);
                filterActive.html(dataTitle);
                $('.score-table').removeClass('active');
                $('.score-table[data-id=' + dataId + ']').addClass('active')
                ...
                //$('.filter-dropdown li ').data('id', 'voleybolligi').trigger('click')
                const dataId = 'voleybolligi';
                const dataTitle = 'Voleybol Efeler Ligi';
                filterActive.parent('.title').attr('data-active', dataId);
                filterActive.html(dataTitle);
                $('.score-table').removeClass('active');
                $('.score-table[data-id=' + dataId + ']').addClass('active')
...
function postToFB(page_title) {
    var width = 600,
        ...
        opts = 'resizable=1,scrollbars=1,status=1' +
            ...
            ',top=' + top +
            ',left=' + left;
    var sharingUrl = ("https://www.facebook.com/sharer/sharer.php?u=" + url + "&text=" + page_title);
...
function postToTWTTR() {
    ...
    var width = 800,
        ...
        opts = 'resizable=1,scrollbars=1,status=1' +
            ...
            ',top=' + top +
            ',left=' + left;
    var articleSharingUrl = "http://twitter.com/intent/tweet?text=" + title + " " + url + "&related=istanbulBBSK";
    var newwindow = window.open(articleSharingUrl, "", opts);
}


const sponsors = $('#sponsors');
const pageWrapper = $('.page-wrapper section');

function menuPosition() {
    const tempScrollTop = $(window).scrollTop();
    const elementTop = sponsors.offset().top - $(window).height();
}


if ($(window).height() < sponsors.offset().top) {
    $(window).scroll(menuPosition);
}

$('#basketboll-tab').click(function (elem) {
    $('.print_voleybol').attr('id', '');
    $('.print_basketbol').attr('id', 'print-page');
});

$('#volleyboll-tab').click(function (elem) {
    $('.print_basketbol').attr('id', '');
    $('.print_voleybol').attr('id', 'print-page');
});
const calendar = $('.calendar');
const calendarHeadMiddleUp = $('.calendar-head-middle-up');
const monthNamesList = ['Ocak', 'Şubat', 'Mart', 'Nisan', 'Mayıs', 'Haziran', 'Temmuz', 'Ağustos', 'Eylül', 'Ekim', 'Kasım', 'Aralık'];
const daysNamesList = ['Pazartesi', 'Salı', 'Çarşamba', 'Perşembe', 'Cuma', 'Cumartesi', 'Pazar'];
$(function () {
    calendar.pignoseCalendar({
        ...
        schedules: eventActives,
        scheduleOptions: {
            colors: {
                event: '#ffffff',
            }
        },
        select: function (date, content, e) {
            const customCalendar = $('.custom-calendar');
            const calendarActivitiesList = $('.calendar-activities-list');
            const calendarHeadMiddleBottom = $('.calendar-head-middle-bottom');
            if (date[0]._f && date[0]._i) {
                const selectMonth = date[0]._pf.parsedDateParts[1];
                let selectDay = ('0' + date[0]._pf.parsedDateParts[2]).slice(-2);
                let dt = new Date(date[0]._i).getDay() - 1;
                if (dt === -1) {
                    dt = 6;
                }
                calendarHeadMiddleUp.html(selectDay + ' ' + monthNamesList[selectMonth] + ' ' + '<span>' + daysNamesList[dt] + '</span>');
                calendarHeadMiddleUp.attr('date', date[0]._i);
                var $this = $(this);
                $('.calendar-activities-list .calendar-item').hide().attr('status', '');
                $('.calendar-activities-list .calendar-item[data-date=' + $this.attr('data-date') + ']').show().attr('status', 'open');

            }
            if ($(this).find('.pignose-calendar-button-schedule-container').length > 0) {
                customCalendar.addClass('close');
                calendarActivitiesList.addClass('open');
                calendarHeadMiddleBottom.html('Düzenlenecek Etkinlikler')
            } else {

                e.stopPropagation()
            }
            $(function () {
                $('.custom-calendar').find('.pignose-calendar-unit').each((index, val) => {
                    customCalendar.addClass('close');
                    calendarActivitiesList.addClass('open');
                    calendarHeadMiddleBottom.html('Düzenlenecek Etkinlikler')
                })
        ...
        init: function (context) {
            calendarHeadMiddleUp.attr('date', context.dateManager.year + '-' + ('0' + context.dateManager.month).slice(-2) + '-' + ('0' + context.dateManager.day).slice(-2));
            calendarHeadMiddleUp.attr('lastDay', ('0' + context.dateManager.lastDay).slice(-2));
            calendarHeadMiddleUp.attr('firstDay', ('0' + context.dateManager.firstDay).slice(-2));
            calendarHeadMiddleUp.attr('month', context.dateManager.month);
            var $this = $(this);
            $('.calendar-activities-list .calendar-item').hide().attr('status', '');
            $('.calendar-activities-list .calendar-item[data-date=' + $this.attr('data-date') + ']').show().attr('status', 'open');
        }
    ...
    function PostDateResult(event) {
        $('.calendar-activities-list .calendar-item').hide().attr('status', '');
        $('.calendar-activities-list .calendar-item[data-date=' + event.target.attr('date') + ']').show().attr('status', 'open');
    }
});
...
if (bodyID === 'home-page') {
    ...
    const featuredNewsSlide = featuredNews.find('.slide');
    const newsListing = $('.news-listing');
    const athletesSlide = $('#home-athletes .slides');
    const olympic = $('#olympic .olympic-head');
    const league = $('#league');
    const league2 = $('#league-2');
    const league3 = $('#league-3');
    const homeArticle = $('#home-branch-article .branch-background');
    const homeGallery = $('.gallerys');
    const playerposition = $('#position-player .player-all');
    const newsList = $('.news-list-wrapper');
    const teamBg = $('.team-bg');
    const tabHeadItem = $('.tab-head-item');
    const content = $('.tab-list-teams .content');
    const topNewsWrapper = $('.top-news-wrapper');
    const scoreResults = $('.score-results');
    const editorSlider = $('.editor-slider');

    function slideHeight() {
        const constant = 657;
        const barNotHeight = Math.max(document.documentElement.clientHeight, window.innerHeight || 0);
        $('.featured-news').find('.slide').css('min-height', +constant + 'px');
        $('.news-list-wrapper').css('max-height', +constant - 76 + 'px');
        //  newsList.css('max-height', +constant + 'px');
    }

    slideHeight();
    if ($(window).width() < 1440 && $(window).width() > 992) {
        const constant = 550;
        featuredNewsSlide.css('min-height', +constant + 'px');
        newsList.css('max-height', +constant - 76 + 'px');
    }
    if ($(window).width() < 992 && $(window).width() > 768) {
        const constant2 = 405;
        featuredNewsSlide.css('min-height', +constant2 + 'px');
    }
    if ($(window).width() < 768) {
        const constant3 = 400;
        featuredNewsSlide.css('min-height','auto');
        featuredNewsSlide.css('height',+constant3 + 'px');
        featuredNewsSlide.css('max-height', +constant3 + 'px');
    }
    ...
    var newsListItem = new Swiper(newsList, {
        slidesPerView: 4,
        spaceBetween: 4,
        direction: 'vertical',
        speed: 500,
    ...
    var featuredSlide = new Swiper('.featured-slide', {
        slidesPerView: 'auto',
        spaceBetween: 0,
        speed: 500,
        autoplay: {
    ...
    league.slick({
        ...
        prevArrow: league.prev().find('.button.left'),
        nextArrow: league.prev().find('.button.right'),
        swipe: true,
        touchMove: true,
        draggable: true,
        responsive: [
    ...
    league2.slick({
        ...
        slidesToShow: 1,
        adaptiveHeight: false,
        prevArrow: league2.prev().find('.button.left'),
        nextArrow: league2.prev().find('.button.right'),
        swipe: true,
        touchMove: true,
        draggable: true,
        responsive: [
    ...
    content.first().show();
    tabHeadItem.on('click', function () {
        const indis = $(this).index();
        tabHeadItem.removeClass('active');
        $(this).addClass('active');
        content.hide();
        content.eq(indis).fadeIn(500)
    })
    topNewsWrapper.slick({
        slidesToShow: 3,
        slidesToScroll: 1,
        adaptiveHeight: true,
        infinite: false,
...
$(document).ready(function () {
    ...
    //Menu Active
    const link = $('.vertical-menu nav a')
    link.click(function () {
    ...
    $(function () {
        var tab = $('.vertical-menu nav a'),
            search_buton = $('.search a');

        tab.hover(function () {
            var data_link = $(this).data('src');
            content.hide();
            $('.menu-content[id=' + data_link + ']').show();
        });
        content.hover(
            function () {
    ...
    const icon_list = $('.search-wrapper .icon-list');
    icon_list.hide()
    $('.custom-search a').on('click', function () {
        if ($('.custom-search a').css("opacity") === '0.25') {
            $('.custom-search a').css('opacity', '1');
            icon_list.fadeToggle();
        } else {
            $('.custom-search a').css('opacity', '0.25');
            icon_list.fadeToggle();
        }
    });

    const icon_list_wrapper = $('.search-wrapper .icon-list .icon-wrapper');
    icon_list_wrapper.on('click', function () {
    ...
    $('.text-search input').keyup(function () {
        if ($('.text-search input').val() != '') {
            ...
            $('.custom-search a').show()
            $('.search-result').hide()
        }
    });


    $('.vertical-menu nav a').hover(function () {
        var content = $('.menu-content')
        if (content.is(':visible')) {
            $('body').css('overflow', 'hidden');
            console.log('deneme')
...
$(document).ready(function () {
    ...
    if (bodyID === 'history-page') {
        $('#history-page .content-slide').slick({
            dots: true,
            arrows: false,
            infinite: false,
            centerPadding: 0,
...
$(document).ready(function () {
    const bodyID = $('body').attr('id');
    if (bodyID === 'president-page') {
        const presidentCircle = $('.president-circles .dates a .circle');

        presidentCircle.first().addClass("activeDate");

        $('.president-images').slick({
            centerMode: true,
            slidesToShow: 3,
            slidesToScroll: 1,
            asNavFor: '.years',
...
$(document).ready(function () {
    ...
    if (bodyID === 'contact-page') {
        const input_act = $('#contact-page input,#contact-page textarea');
        input_act.keyup(function () {
    ...
    if (bodyID === 'score-page') {
        ...
        team.on('click', function () {
            if ($(this).hasClass('score-active')) {
                ...
                $(this).next().removeClass('open-detail')
            } else {
                team.removeClass('score-active');
                team.next().removeClass('open-detail');
                $(this).toggleClass('score-active');
                $(this).next().toggleClass('open-detail');
            }
        });
    }
});
        ...
        $.ajax({
            ...
            success: function (res) {
                setTimeout(function () {
                    ...
                    $('.comment-alert.default').hide();
                    $('.comment-alert.success').removeClass('hidden');
                }, 300)
            },
            error: function (res) {
                console.log('ERROR');
...
$(document).ready(function () {
    const bodyID = $('body').attr('id');
    const userButton = $('#comment-form button');
    const inputName = $('#comment-form input[name=name]');
    const inputEmail = $('#comment-form input[name=email]');
    const inputMessage = $('#comment-form textarea[name=message]');
    if (bodyID === 'article-page') {
        $('#article-page .slide').slick({
            dots: true,
            arrows: false,
            infinite: false,
            centerPadding: 0,
        ...
        inputName.on('keyup', function (e) {
            ...
            if ($(this).val().length > 1) {
                inputEmail.removeAttr('disabled').parent('label').addClass('active');
            } else {
                inputEmail.attr('disabled', 'disabled').val('').parent('label').removeClass('active');
                inputMessage.attr('disabled', 'disabled').val('').parent('label').removeClass('active');
                userButton.attr('disabled', 'disabled').removeClass('active');
            }
            if (code === '9') {
                inputEmail.removeAttr('disabled').parent('label').addClass('active').focus();
            }
        });
        inputEmail.on('keyup', function (e) {
            const code = e.keyCode || e.which;
            if ($(this).val().length > 1) {
                inputMessage.removeAttr('disabled').parent('label').addClass('active');
            } else {
                inputMessage.attr('disabled', 'disabled').val('').parent('label').removeClass('active');
            }
            if (code === '9') {
                inputMessage.removeAttr('disabled').parent('label').addClass('active').focus();
            }
        });
        inputMessage.on('keyup', function () {
            if ($(this).val().length > 1) {
                userButton.removeAttr('disabled').addClass('active');
            } else {
                userButton.attr('disabled', 'disabled').removeClass('active');
            }
        });
...
const filterDate = $('.calendar_format');
const articlesLoadButton = $('.articles-list button');
filterDate.pignoseCalendar({
    ...
    click(event, context) {
        if (context.element.val() !== '') {
            context.element.parent('label').addClass('active');
        } else {
            context.element.parent('label').removeClass('active');
        }
    }
...
$('#article-filter').on('submit', function (e) {
    var valueInput = $('input[id=custom_search]').val()
    console.log(valueInput)
    var urlName = document.location.pathname;
    document.location.href  = urlName + "?search=" + valueInput;
    $('.education-items ul').html('');
    e.preventDefault();
    e.stopPropagation();
    nextPage = 1;
    let first_date = $('input[name=published_date_before]').val();
    let last_date = $('input[name=published_date_after]').val();
    if (first_date && last_date) {
        GoItem();
    } else if (!first_date && last_date) {
        $('input[name=published_date_before]').parent('label').css('border-color', 'red');
    } else if (first_date && !last_date) {
        $('input[name=published_date_after]').parent('label').css('border-color', 'red');
    } else {
        GoItem();
    }
});
...
function GoItem() {
    let first_date = $('input[name=published_date_before]').val();
    let last_date = $('input[name=published_date_after]').val();
    let branch_list = $('select[name=branch_id] option:selected').val();
    let custom_search = $('input[name=search]').val();
    const containerResult = $('.education-items ul');
    ...
    $.ajax({
        ...
        data: {
            page_size: 12,
            page: nextPage,
            published_date_before: last_date,
            published_date_after: first_date,
            branch_id: branch_list,
            search: custom_search,
        },
        success: function (res) {
            if (!res.next) {
                articlesLoadButton.hide();
            } else {
                articlesLoadButton.show();
            }
            let results = "";
            const news = res.results;
            news.forEach(function (data) {
                const newDate = new Date(data.published_date);
                let dataContent = data.content.replace(/<[^>]+>/g, '');
                let view = "";
                if (data.view) {
                    view = '<img src="/static/img/icons/eye-white.svg" alt="eye">' + data.view + '</span>\n'
                }
                results += '' +
                    '<li>\n' +
                    '<a href="' + data.get_absolute_url + '" title="' + data.title + '"\n' +
                    'style="background-image: url(' + data.featured_photos + ')">\n' +
                    '<div class="not-hover">\n' +
                    '<div class="title">' + data.title + '</div>\n' +
                    '<div class="icon">\n' +
                    '<img src="' + data.branch.icon + '" alt="education-book"><b>' + data.branch.name + '</b>\n' +
                    '</div>\n' +
                    '</div>\n' +
                    '<div class="hover">\n' +
                    '<div class="date">\n' +
                    '<time>\n' +
                    '<img src="/static/img/icons/time-white.svg" alt="time">\n' +
                    '<span>' + ('0' + newDate.getDate()).slice(-2) + ' ' + monthNames[newDate.getMonth()] + ' ' + newDate.getFullYear() + ' ' + newDate.getHours() + ':' + newDate.getMinutes() + '</span>\n' +
                    '</time>\n' +
                    '<span class="displayed">' +
                    view +
                    '</div>\n' +
                    '<div class="title">' + data.title + '</div>\n' +
                    '<p>' + dataContent.substring(0, 120) + '...</p>\n' +
                    '<div class="read-more">\n' +
                    '<div class="icon">\n' +
                    '<img src="' + data.branch.icon + '" alt="education-book"><b>' + data.branch.name + '</b>\n' +
                    '</div>\n' +
                    '<div class="link">Devamını Oku</div>\n' +
                    '</div>\n' +
                    '</div>\n' +
                    '</a>\n' +
                    '</li>\n';
            ...
            if (!news.length < 1) {
                containerResult.append(results);
            } else {
                containerResult.html('<p class="alert alert-warning text-center" style="width: 100%;">Hiç bir sonuç bulunamadı.</p>');
            }
        },
        error: function (res) {
        }
...
articlesLoadButton.click(function () {
    GoItem();
});
...
function load_articles() {
    ...
    $.get(pathname + 'get-articles?sayfa=' + page).done(function (res) {
        $.each(res.data, function (i, article) {
            var articleLimit;
            var articleContent = article.content.replace(/<[^>]+>/g, '');
            if (articleContent.length > 170) {

            }
            article_html = '<a href="' + article.url + '" title="' + article.title + '" style="background: url(' + article.image + ') no-repeat center center; background-size: cover"\n' +
                '                     class="background-color">\n' +
                '                    <div class="branch-article">\n' +
                '                        <div class="branch-article-up">\n' +
                '                            <div class="branch-article-time">\n' +
                '                                <p><i class="fa fa-clock-o clock" aria-hidden="true"></i>' + article.date +
                '                                </p>\n' +
                '                            </div>\n' +
                '                            <div class="branch">\n' +
                '                                <p>' + article.branch + '</p>\n' +
                '                            </div>\n' +
                '                        </div>\n' +
                '                        <div class="branch-article-bottom">\n' +
                '                            <p class="branch-article-title">' + article.title + '</p>\n' +
                '                            <div class="views-readmore">\n' +
                '                                <div class="branch-article-views">\n' +
                '                                    <img src="/static/img/eye.svg" alt="views">\n' +
                '                                    <span>' + article.view + '</span>\n' +
                '                                </div>\n' +
                '                                <div class="readMore" title="Devamını Oku">Devamını oku</div>\n' +
                '                            </div>\n' +
                '                        </div>\n' +
                '                    </div>\n' +
                '                </a>';
            $('.branch-background').append(article_html)
        });
        if (res.has_next) {
            page++
        } else {
...
if (window.location.pathname.indexOf('/brans/') > -1) {
    ...
    function sliderMake() {
        const branchBackground = $('.branch-background');
        if ($(window).width() < 992) {
            setTimeout(function () {
                branchBackground.slick({
                    slidesToShow: 3.2,
                    slidesToScroll: 2,
                    infinite: false,
                    adaptiveHeight: true,
...
$(document).ready(function () {
    const websiteSearch = $('#website_search, #mobile_search');
    const FilterSearch = $('.filter-search');
    let Results = $('#results');
    // websiteSearch.on('keyup', function () {
    //     let thisValues = $(this).val();
    //     Results.removeClass('hidden');
    //     FilterSearch.removeClass('active');
    //     if (thisValues.length >= 3) {
    //         if ($(this)[0].id === 'website_search') {
    //             Results = $('#results');
    //             FilterSearch.removeClass('active');
    //             $.ajax({
    //                 url: "/arama/search?term=" + thisValues,
    //                 method: 'GET',
    //                 success: function (res) {
    //                     let result = '';
    //                     const keys = Object.keys(res);
    //                     keys.forEach(k => {
    //                         result += `<li><a href="${res[k].url}" title="${res[k].title}">
    //                                         <span class="img hidden">
    //                                             <img src="/static/img/homepage/icon/ball.svg" alt="ball">
    //                                         </span>
    //                                         <b>${res[k].tag}${res[k].title}</b>
    //                                     </a>
    //                                 </li>`;
    //                     });
    //                     Results.find('ul').html(result);
    //
    //                 }
    //             });
    //
    //         } else if ($(this)[0].id === 'mobile_search') {
    //             Results = $('.results');
    //             $.ajax({
    //                 url: "/arama/search?term=" + thisValues,
    //                 method: 'GET',
    ...
    //                     const keys = Object.keys(res);
    //                     keys.forEach(k => {
    //                         result += `<a href="${res[k].url}" title="${res[k].title}"><b>${res[k].tag}${res[k].title}</b><i class="fa fa-angle-right"></i></a>`;
    //                     });
    //                     Results.html(result);
    //                 }
    //             });
...
$(document).ready(function () {
    const galleryCategory = $('.gallery-content button');
    const containerPhoto = $('.gallery-images');
    let count = 1;
    let orderBy = $('.head-links .links a.active').data('ordering');
    let data_url;

    galleryCategory.on('click', function () {
        // galleryCategory.removeClass('active');
        // $(this).addClass('active');
        data_url = `/api/gallery-list?page_size=20&&ordering=${orderBy}&&page=`;
        $.ajax({
            url: data_url + count.toString(),
            method: 'GET',
            dataType: 'json',
            success: function (res) {
                let result = '';
                ...
                count += 1;
                containerPhoto.append(result);
                if (!res.next) {
                    $(galleryCategory).css('display', 'None');
                }
            }, error: function (res) {
            }
        });
    ...
    setTimeout(function () {
        galleryCategory.trigger('click');
    }, 1500);

    $('.head-links .links a').on('click', function () {
        let newOrderBy = $('.head-links .links a.active').data('ordering');
...
$(document).ready(function () {
    const videos_button = $('.video-listing button');
    const Videocontainer = $('.video-listing ul');
    let result = '', count = 1, url = '';

    videos_button.on('click', function () {
        result = '';
        url == 'videos' ? loadSelected() : loadAll();
    });

    function loadSelected() {
        $.ajax({
            url: '/api/videos/?page_size=12&&page=' + count.toString(),
            method: 'GET',
            dataType: 'json',
            success: function (res) {
                let result = '';
                const Videocontainer = $('.video-listing ul');
                res.results.forEach(function (item) {
                    if (!item.featured_photo.length < 1) {
                        branch_elem = '';
                        if (item.branch) {
                            branch_elem = `<div class="branch">
                                    <img src="${item.branch.icon}" alt="ball">
                                    <b>${item.branch.name}</b>
                             </div>`;
                        }
                        result += `<li>
                                        <a href="${item.embed}"
                                           title="${item.title}" data-fancybox
                                           style="background-image: url(${item.featured_photo})">
                                            <div class="content">
                                                <div class="top">${branch_elem}</div>
                                                <div class="play" title="Başlat"><img
                                                        src="/static/img/icons/youtube-icon.svg" alt="youtube-icon">
                                                </div>
    ...
    function loadAll() {
        $.ajax({
            url: '/api/youtube-videos/?page_size=12&&page=' + count.toString(),
            method: 'GET',
            dataType: 'json',
            success: function (res) {
                res.results.forEach(function (item) {
                    result += `<li>
                                        <a href="https://www.youtube.com/watch?v=${item.video_id}"
                                           title="${item.title}" data-fancybox
                                           style="background-image: url('https://img.youtube.com/vi/${item.video_id}/hqdefault.jpg')">
                                            <div class="content">
                                                <div class="top"></div>
                                                <div class="play" title="Başlat"><img
                                                        src="/static/img/icons/youtube-icon.svg" alt="youtube-icon">
    ...
    setTimeout(function () {
        url = $('.links a.active').data('url');
        videos_button.trigger('click');
    }, 1500);

    $('.links a').on('click', function () {
        new_url = $('.links a.active').data('url');
        if (new_url != url) {
            Videocontainer.html('');
            videos_button.show();
            url = new_url;
...
$(document).ready(function () {
    ...
    const CalendarSearch = $('#activities-filter #custom_search');
    const CalendarSelect = $('#activities-filter select');
    const CalendarInput = ActivitiesFilter.find('.calendar');
    CalendarInput.pignoseCalendar({
        theme: 'light',
        format: 'DD/MM/YYYY',
        week: 1,
        weeks: [
    ...
    if (bodyId === 'activities-page') {
        const Calendar = $('#activity-calendar');
        const CalendarDays = $('#calendar-days');
        const CalenderSubmit = $('#activities-filter button');
        const DateTime = Calendar.find('#date time');
        const CalendarMonth = Calendar.find('#month li');
        const PostDate = $('#post-date');
        const CalendarOpen = $('.calendar-open');
        CalenderSubmit.on('click', function (e) {
            const data = $('#activities-filter form').serializeArray();
            $.ajax({
                url: '',
                method: 'GET',
                dataType: 'json',
        ...
        CalendarDays.pignoseCalendar({
            ...
            select: function (date, content) {
                if (date[0]._f && date[0]._i) {
                    ...
                    DateTime.html(selectDay + ' ' + monthNames[selectMonth] + ' ' + '<span>' + daysNames[dt] + '</span>');
                    $('#month li[data-month=' + selectMonth + ']').addClass('active');
                    DateTime.attr('date', date[0]._i);
                }
            },
            init: function (context) {
                DateTime.attr('date', context.dateManager.year + '-' + ('0' + context.dateManager.month).slice(-2) + '-' + ('0' + context.dateManager.day).slice(-2));
                DateTime.attr('lastDay', ('0' + context.dateManager.lastDay).slice(-2));
                DateTime.attr('firstDay', ('0' + context.dateManager.firstDay).slice(-2));
                DateTime.attr('month', context.dateManager.month);
            }
        });
        Calendar.find('#date .button').on('click', function (e) {
            let toggleDate;
            const dateTimeSplit = parseInt(DateTime.attr('date').split('-')[2]);
            const clickPrev = dateTimeSplit - 1;
            const clickNext = dateTimeSplit + 1;
            if (e.currentTarget.className === 'button prev') {
                toggleDate = clickPrev;
                if (toggleDate < DateTime.attr('firstDay')) {
                    toggleDate = DateTime.attr('firstDay');
                }
            } else if (e.currentTarget.className === 'button next') {
                toggleDate = clickNext;
                if (toggleDate > DateTime.attr('lastDay')) {
                    toggleDate = DateTime.attr('lastDay');
                }
            }
            PostDateResult();
            setTimeout(function () {
                PostDate.trigger('click');
            }, 300);
            CalendarDays.pignoseCalendar('set', toggleDate.toString());
            DateTime.attr('date', $('.pignose-calendar-unit-first-active').attr('data-date'));
            let dt = new Date(DateTime.attr('date')).getDay() - 1;
            if (dt === -1) {
                dt = 6;
            }
            DateTime.html(toggleDate + ' ' + monthNames[DateTime.attr('month') - 1] + ' ' + '<span>' + daysNames[dt] + '</span>');
        });
        CalendarMonth.on('click', function () {
            const thisMonth = $(this).data('month');
            DateTime.attr('date', thisMonth);
            DateTime.attr('month', thisMonth.split('/')[1]);
            CalendarDays.pignoseCalendar('set', thisMonth.toString());
            CalendarMonth.removeClass('active');
            $(this).addClass('active');
        });
        CalendarOpen.on('click', function () {
            $(this).removeClass('active');
            $('#con_one').removeClass('active');
            $('#con_two').addClass('active');
            CalendarDays.show();
            PostDate.show();
        });

        function PostDateResult() {
            $('.result-listing li.activity').hide().attr('status', '');
            $('.result-listing li[data-date=' + DateTime.attr('date') + ']').show().attr('status', 'open');
            $('.result.length').text('Bu tarihte toplam ' + $('.result-listing li[status="open"]').length + ' etkinlik bulunmaktadır')
        }
        ...
        PostDate.on('click', function () {
            CalendarDays.hide();
            $('#con_two').removeClass('active');
            $('#con_one').addClass('active');
            CalendarOpen.addClass('active');
            PostDateResult();
            $(this).hide();
        });
    }
...
$(document).ready(() => {
    const pignoseCalendarUnit = $('.pignose-calendar-unit');
    const customCalendar = $('.custom-calendar');
    const calendarActivitiesList = $('.calendar-activities-list');
    const calendarHeadMiddleUp = $('.calendar-head-middle-up');
    const calendarHeadMiddleBottom = $('.calendar-head-middle-bottom');
    const pignoseCalendarTopMonth = $('.pignose-calendar-top-month');
    const pignoseCalendarTopYear = $('.pignose-calendar-top-year');
    const pignoseCalendarTopPrev = $('.pignose-calendar-top-prev');
    const pignoseCalendarTopNext = $('.pignose-calendar-top-next')
    const calendarHeadRight = $('.calendar-head-right');
    const calendarHeadLeft = $('.calendar-head-left');

    $(function () {
        $('.custom-calendar').find('.pignose-calendar-unit').each((index, val) => {
            var active = $(this).find('.pignose-calendar-button-schedule-container');
            if (active.length > 0) {
                active.siblings('a').addClass('active')
            }
        })
    })

    calendarHeadMiddleBottom.html(pignoseCalendarTopMonth.text() + '/' + pignoseCalendarTopYear.text())
    $(function () {
        $('.custom-calendar').find('.pignose-calendar-unit').each((index, val) => {
            var active = $(this).find('.pignose-calendar-button-schedule-container');
            if (active.length > 0) {
    ...
    // pignoseCalendarUnit.on('click', () => {
    //     customCalendar.addClass('close');
    //     calendarActivitiesList.addClass('open');
    //     calendarHeadMiddleBottom.html('Düzenlenecek Etkinlikler')
    // });
    calendarActivitiesList.hover(function () {

    }, function () {
        $(this).removeClass('active');
    });
    calendarActivitiesList.mouseenter(function () {

    }).mouseleave(function () {
        calendarActivitiesList.removeClass('open');
        customCalendar.removeClass('close');
        calendarHeadMiddleUp.html('Etkinlik Takvimi');
        calendarHeadMiddleBottom.html(pignoseCalendarTopMonth.text() + '/' + pignoseCalendarTopYear.text());
        $(function () {
            $('.custom-calendar').find('.pignose-calendar-unit').each((index, val) => {
                var active = $(this).find('.pignose-calendar-button-schedule-container');
    ...
    calendarHeadLeft.on('click', () => {
        pignoseCalendarTopPrev.trigger('click');
        calendarHeadMiddleBottom.html(pignoseCalendarTopMonth.text() + '/' + pignoseCalendarTopYear.text());
        calendarActivitiesList.removeClass('open');
        customCalendar.removeClass('close');
        calendarHeadMiddleUp.html('Etkinlik Takvimi');
    ...
    calendarHeadRight.on('click', () => {
        pignoseCalendarTopNext.trigger('click');
        calendarHeadMiddleBottom.html(pignoseCalendarTopMonth.text() + '/' + pignoseCalendarTopYear.text());
        calendarActivitiesList.removeClass('open');
        customCalendar.removeClass('close');
        calendarHeadMiddleUp.html('Etkinlik Takvimi');
...
//$('select#branch_list').change(function () {
//    var filter = $(this).val();
//    filterList(filter);
//});
//
...
//    $(list).hide();
//    if (value == "All") {
//        $(".recruiter").find("li").each(function (i) {
//            $(this).show();
//        });
//    } else {
//        $(".recruiter").find("li[data-custom-type*=" + value + "]").each(function (i) {
//            $(this).show();
//        });
//    }
//}
function print() {
    const w = window.open();
    w.document.write($('#print-page').html());
    w.print();
    w.close();
}
